...
data_root = "/project/hnguyen2/mvu9/datasets/SEN1Floods11/"
...
splits = {
    "train": f"data_splits/sen1floods11/train_{pc_label}.txt",
...
pretrained_weights_path = "/project/hnguyen2/mvu9/datasets/SEN1Floods11/v1.1/checkpoints/5e4_flood_0_89_0.5418742895126343.cp"
num_layers = 12
patch_size = 16
embed_dim = 768
num_heads = 12
...
optimizer = dict(
    type="AdamW",
    lr=3e-5,  # Adjusted for full tuning
    weight_decay=0.05,
    betas=(0.9, 0.999),
)
...
evaluation = dict(
    ...
    metric=["mIoU", "mDice"],  # Added mDice for evaluation
    pre_eval=True,
    save_best="mIoU",
    by_epoch=True,
)
...
model = dict(
    ...
    backbone=dict(
        type="TemporalViTEncoder",  # Changed to standard backbone
    ...
    neck=dict(
        type="ConvTransformerTokensToEmbeddingNeck",  # Changed to standard neck
        embed_dim=1 * 768,
